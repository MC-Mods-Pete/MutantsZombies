...
public class ModRangedAttackGoal extends Goal {
    ...
    public ModRangedAttackGoal(RangedAttackMob pRangedAttackMob, double pSpeedModifier, int pAttackInterval, float pAttackRadius) {
        this(pRangedAttackMob, pSpeedModifier, pAttackInterval, pAttackInterval, pAttackRadius);
    }

    public ModRangedAttackGoal(RangedAttackMob pRangedAttackMob, double pSpeedModifier, int pAttackIntervalMin, int pAttackIntervalMax, float pAttackRadius) {
        if (!(pRangedAttackMob instanceof LivingEntity)) {
            throw new IllegalArgumentException("ArrowAttackGoal requires Mob implements RangedAttackMob");
        } else {
            this.rangedAttackMob = pRangedAttackMob;
            this.mob = (MobEntity) pRangedAttackMob;
     ...
     */
    public void tick() {
        double d0 = this.mob.squaredDistanceTo(this.target.getX(), this.target.getY(), this.target.getZ());
        boolean flag = this.mob.getVisibilityCache().canSee(this.target);
        if (flag) {
            ++this.seeTime;
        } else {
            this.seeTime = 0;
        ...
        if (!(d0 > (double)this.attackRadiusSqr) && this.seeTime >= 5) {
            ...
            this.mob.getNavigation().startMovingTo(this.target, this.speedModifier);
        }
